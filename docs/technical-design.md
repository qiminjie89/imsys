...
| 消息 QPS | 200 | 500 | 500 |
| 平均消息大小 | 2KB | 2KB | 2KB |
| 单房间用户数 | 1,000 | 1,000 | 5,000 |
| 总在线用户 | 10,000 | 30,000 | 50,000 |
...
| 场景 | 扇出消息/秒 | 出向带宽 |
|------|------------|---------|
| 日常 | 200 × 500 = 100K | 100K × 2KB = **200MB/s = 1.6Gbps** |
| 高峰 | 500 × 1000 = 500K | 500K × 2KB = **1GB/s = 8Gbps** |
| 大房间 | 500 × 5000 = 2.5M | 2.5M × 2KB = **5GB/s = 40Gbps** |

> 注：实际场景中消息分布在多个房间，上表按热点房间最大值估算

#### 实例规划

| 组件 | 日常 | 高峰 | 大房间 | 说明 |
|------|------|------|--------|------|
| **Gateway** | 2 | 3 | 6 | 每实例 ~15K 连接，10Gbps 网卡 |
| **Roomserver** | 1 | 2 | 2 | 按 room_id 分片 |
| **Dispatcher** | 1 | 2 | 2 | 无状态，可水平扩展 |
| **Kafka** | 3 broker | 3 broker | 5 broker | 3 分区起步 |

#### 单机配置建议

| 组件 | CPU | 内存 | 网卡 | 说明 |
|------|-----|------|------|------|
| Gateway | 4核 | 4GB | **10Gbps** | 主要消耗：序列化、扇出 |
| Roomserver | 2核 | 2GB | 1Gbps | 纯内存状态，轻量 |
| Dispatcher | 1核 | 512MB | 1Gbps | HTTP API，极轻量 |
...
|-------------|---------|-----------|
| < 2Gbps | 5Gbps | 标准型 |
| 2-8Gbps | **10Gbps** | 网络增强型（推荐） |
| 8-25Gbps | 25Gbps | 高性能计算型 |
| > 25Gbps | 多网卡绑定 | 需特殊配置 |

#### 华为云流量成本估算（2024参考价）

**假设：日常 8 小时高负载 + 2 小时活动高峰**

| 场景 | 带宽 | 日流量 | 月流量 | 月成本(按量) |
|------|------|--------|--------|-------------|
| 日常 8h | 200MB/s | 5.6TB | 168TB | - |
| 高峰 2h | 1GB/s | 7.2TB | 216TB | - |
| **合计** | - | 12.8TB | **384TB** | ~¥230,000 |

**优化方案：购买带宽包**

| 方案 | 配置 | 月成本 | 节省 |
|------|------|--------|------|
| 按量计费 | 384TB/月 | ¥230,000 | - |
| **共享带宽包** | 10Gbps × 2 | ~¥80,000 | **65%** |
| 95计费 | 峰值 8Gbps | ~¥60,000 | **74%** |

> 建议：使用共享带宽包或 95 计费，避免按流量计费
